...
/**
 * Handles messages that consist of a format string containing '{}' to represent
 * each replaceable token, and the parameters.
 * <p>
 * This class was originally written for <a
 * href="http://lilithapp.com/">Lilith</a> by Joern Huxhorn where it is licensed
 * under the LGPL. It has been relicensed here with his permission providing
...
public class ParameterizedMessage implements Message, Serializable {
     ...
     *            The message "format" string. This will be a String containing
    ...
    /**
     * Constructs a ParameterizedMessage which contains the arguments converted
     * to String as well as an optional Throwable.
     *
     * <p>
     * If the last argument is a Throwable and is NOT used up by a placeholder
     * in the message pattern it is returned in {@link #getThrowable()} and
     * won't be contained in the created String[]. If it is used up
     * {@link #getThrowable()} will return null even if the last argument was a
     * Throwable!
     * </p>
     *
     * @param messagePattern
     *            the message pattern that to be checked for placeholders.
     * @param arguments
     ...
     * will not survive serialization. The Throwable exists as part of the
     * message primarily so that it can be extracted from the end of the list of
     * parameters and then be added to the LogEvent. As such, the Throwable in
     * the event should not be used once the LogEvent has been constructed.
     *
     ...
     *
     * @param messagePattern
     *            the message pattern containing placeholders.
     * @param arguments
    ...
    /**
     * Detects whether the message pattern has been fully processed or if an
     * unprocessed character remains and processes it if necessary, returning
     * the resulting position in the result char array.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    ...
    /**
     * Processes the last unprocessed character and returns the resulting
     * position in the result char array.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    // 28 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
    ...
    /**
     * Processes a literal char (neither an '\' escape char nor a "{}" delimiter
     * pair) and returns the resulting position.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    ...
    /**
     * Writes "{}" to the specified result array at the specified position and
    ...
    /**
     * Writes a '\' char to the specified result array (starting at the
     * specified position) for each <em>pair</em> of '\' escape chars
     * encountered in the message format and returns the resulting position.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    // 11 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
    ...
    /**
     * Writes the specified number of '\' chars to the specified result array
     * (starting at the specified position) and returns the resulting position.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    ...
    /**
     * Appends the argument at the specified argument index (or, if no such
     * argument exists, the "{}" delimiter pair) to the specified result char
     * array at the specified position and returns the resulting position.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    ...
    /**
     * Appends the argument at the specified argument index to the specified
     * result char array at the specified position and returns the resulting
     * position.
     */
    // Profiling showed this method is important to log4j performance. Modify with care!
    // 30 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
    ...
    /**
     * This method performs a deep toString of the given Object. Primitive
     * arrays are converted using their respective Arrays.toString methods while
     * special handling is implemented for "container types", i.e. Object[], Map
     * and Collection because those could contain themselves.
     * <p>
     * It should be noted that neither AbstractMap.toString() nor
     * AbstractCollection.toString() implement such a behavior. They only check
     * if the container is directly contained in itself, but not if a contained
     * container contains the original one. Because of that,
     * Arrays.toString(Object[]) isn't safe either. Confusing? Just read the
     * last paragraph again and check the respective toString() implementation.
     * </p>
     * <p>
     * This means, in effect, that logging would produce a usable output even if
     * an ordinary System.out.println(o) would produce a relatively
     * hard-to-debug StackOverflowError.
     * </p>
     * 
     * @param o
     *            The object.
     ...
     * @param str
     *            the StringBuilder that o will be appended to
     * @param dejaVu
    ...
    /**
     * Returns {@code true} if the specified object is an array, a Map or a
     * Collection.
     */
     ...
     * </p>
     * <blockquote> As much as is reasonably practical, the hashCode method
     * defined by class {@code Object} does return distinct integers for
