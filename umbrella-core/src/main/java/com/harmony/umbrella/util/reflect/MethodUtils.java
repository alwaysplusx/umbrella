...
public abstract class MethodUtils {
	...
	/**
	 * 在指定类以及父类中查找指定名称无参数的方法,<p> Attempt to find a {@link Method} on the supplied class
	 * with the supplied name and no parameters. Searches all superclasses up to
	 * {@code Object}. <p>Returns {@code null} if no {@link Method} can be found.
	 * 
	 * @param source
	...
	/**
	 * 在指定类以及其父类中查找名称以及参数类型相同的的方法<p> Attempt to find a {@link Method} on the supplied
	 * class with the supplied name and parameter types. Searches all superclasses up to
	 * {@code Object}. <p>Returns {@code null} if no {@link Method} can be found.
	 * 
	 * @param source
	 *            the class to introspect
	 ...
	 *            the name of the method
	 * @param paramTypes
	 *            the parameter types of the method (may be {@code null} to indicate any
	 *            signature)
	 * @return the Method object, or {@code null} if none found
	 */
	public static Method findMethod(Class<?> source, String name, Class<?>... paramTypes) {
		...
		while (searchType != null) {
			...
			for (Method method : methods) {
				if (name.equals(method.getName()) && ClassUtils.typeEquals(method.getParameterTypes(), paramTypes)) {
					return method;
				}
			}
	...
	public static Method[] findMethods(Class<?> source, Class<?>[] parameterTypes) {
		...
		while (searchType != null) {
			Method[] methods = (searchType.isInterface() ? searchType.getMethods() : getDeclaredMethods(searchType));
			for (Method method : methods) {
				if (ClassUtils.typeEquals(method.getParameterTypes(), parameterTypes)) {
					result.add(method);
				}
			}
			searchType = searchType.getSuperclass();
	...
	public static Object invokeMethod(String methodName, Object target) {
		return invokeMethod(findMethod(target.getClass(), methodName), target);
	}
	 ...
	 * {@link Method}. <p>Thrown exceptions are handled via a call to
	...
	/**
	 * Invoke the specified JDBC API {@link Method} against the supplied target object
	 * with no arguments.
	 * 
	 * @param method
	 *            the method to invoke
	 ...
	 * superclasses. <p>The same named method occurring on subclass and superclass will
	...
	/**
	 * Perform the given callback operation on all matching methods of the given class and
	 * superclasses (or given interface and super-interfaces). <p>The same named method
	 * occurring on subclass and superclass will appear twice, unless excluded by the
	 * specified {@link MethodFilter}.
	 * 
	 * @param clazz
	 *            the class to introspect
	 * @param mc
	...
	/**
	 * This variant retrieves {@link Class#getDeclaredMethods()} from a local cache in
