...
/**
 * 断言工具类 <p> Assertion utility class that assists in validating arguments.
 ...
 * is deemed invalid, an {@link IllegalArgumentException} is thrown (typically).
 * For example:
 * <p/>
 * <pre class="code"> Assert.notNull(clazz, "The class must not be null");
...
public abstract class Assert {
     ...
     * the test result is {@code false}. <pre class="code">Assert.isTrue(i &gt;
    ...
    /**
     * Assert that an object is {@code null} . <pre
    ...
    /**
     * Assert that an object is not {@code null} . <pre
     ...
     * {@code null} and not the empty String. <pre
     ...
     * and must have at least one element. <pre
     ...
     * {@code null} and must have at least one element. <pre
     ...
     * must have at least one entry. <pre class="code">Assert.notEmpty(map,
    ...
    /**
     * Assert that a Map has entries; that is, it must not be {@code null} and
     * must have at least one entry. <pre
     ...
     * IllegalArgumentException on an assertion failure. <pre
     ...
     * the test result is {@code false}. <p>Call {@link #isTrue(boolean)} if you
     * wish to throw {@link IllegalArgumentException} on an assertion failure.
