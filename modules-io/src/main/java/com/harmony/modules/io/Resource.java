 ...
 *
 * <p>An InputStream can be opened for every resource if it exists in physical
...
public interface Resource {
    ...
    /**
     * Return whether this resource actually exists in physical form. <p>This
     * method performs a definitive existence check, whereas the existence of a
     ...
     * {@link #getInputStream()} or {@link #getFile()}. <p>Will be {@code true}
     * for typical resource descriptors; note that actual content reading may
     * still fail when attempted. However, a value of {@code false} is a
     ...
     * closed to avoid resource leaks. <p>Will be {@code false} for typical
     ...
     * the path: for example, "myfile.txt". <p>Returns {@code null} if this type
     * of resource does not have a filename.
     */
    String getFilename();

    /**
     * Return a description for this resource, to be used for error output when
     * working with the resource. <p>Implementations are also encouraged to
     * return this value from their {@code toString} method.
     * 
     * @see Object#toString()
     */
    ...
    /**
     * Return an {@link InputStream}. <p>It is expected that each call creates a
     * <i>fresh</i> stream. <p>This requirement is particularly important when
     * you consider an API such as JavaMail, which needs to be able to read the
     * stream multiple times when creating mail attachments. For such a use
     * case, it is <i>required</i> that each {@code getInputStream()} call
