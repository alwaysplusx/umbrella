...
public interface PathMatcher {
     ...
     * implementation of this interface? <p>If the return value is {@code false}
     * , then the {@link #match} method does not have to be used because direct
     * equality comparisons on the static path Strings will lead to the same
     * result.
     * 
     * @param path
     ...
     * the actually pattern-matched part of the path. <p>For example: For
     * "myroot/*.html" as pattern and "myroot/myfile.html" as full path, this
     * method should return "myfile.html". The detailed determination rules are
     * specified to this PathMatcher's matching strategy. <p>A simple
     * implementation may return the given full path as-is in case of an actual
     * pattern, and the empty String in case of the pattern not containing any
     * dynamic parts (i.e. the {@code pattern} parameter being a static path
     ...
     * <p>For example: For pattern "/hotels/{hotel}" and path "/hotels/1", this
     ...
     * patterns in order of explicitness for that path. <p>The full algorithm
     * used depends on the underlying implementation, but generally, the
     * returned {@code Comparator} will
    ...
    /**
     * Combines two patterns into a new pattern that is returned. <p>The full
     * algorithm used for combining the two pattern depends on the underlying
     * implementation.
     * 
     * @param pattern1
     *            the first pattern
