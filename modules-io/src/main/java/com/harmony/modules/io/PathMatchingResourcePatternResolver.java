 ...
 */
package com.harmony.modules.io;
...
import com.harmony.modules.io.utils.AntPathMatcher;
import com.harmony.modules.io.utils.PathMatcher;
 ...
 * class="code"> /WEB-INF/*-context.xml
 * com/mycompany/**&#47;applicationContext.xml file:C:/some/path/*-context.xml
 ...
 * segment and obtains a {@code URL} from it. If this URL is not a "{@code jar:}
 * " URL or container-specific variant (e.g. "{@code zip:}" in WebLogic, "
 * {@code wsjar}" in WebSphere", etc.), then a {@code java.io.File} is obtained
 * from it, and used to resolve the wildcard by walking the filesystem. In the
 ...
 * implicitly because the base {@code ResourceLoader} is a filesystem one, then
 ...
 * must be able to get a {@code java.net.JarURLConnection} from it, or manually
 * parse the jar URL, to be able to walk the contents of the jar, and resolve
 * the wildcard. This will work in most environments, but will fail in others,
 * and it is strongly recommended that the wildcard resolution of resources
 * coming from jars be thoroughly tested in your specific environment before you
 * rely on it.
 *
 ...
 *
 * <p>The "classpath*:" prefix can also be combined with a PathMatcher pattern
 ...
 * loader hierarchy, and then off each resource the same PathMatcher resolution
 ...
 * as <pre class="code"> com/mycompany/package1/service-context.xml </pre> may
 * be in only one location, but when a path such as <pre class="code">
 * classpath:com/mycompany/**&#47;service-context.xml </pre> is used to try to
 * resolve it, the resolver will work off the (first) URL returned by
 * {@code getResource("com/mycompany");}. If this base package node exists in
 * multiple classloader locations, the actual end resource may not be
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
    ...
    /**
     * Create a new PathMatchingResourcePatternResolver. <p>ClassLoader access
     * will happen via the thread context class loader.
     * 
     ...
     * {@link Resource}. <p>The default implementation simply creates a
     * {@link UrlResource} instance.
     * 
     * @param url
                    ...
                    if (ResourceUtils.isJarFileURL(url)) {
                        try {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug("Cannot search for matching files underneath [" + url + "] because it cannot be converted to a valid 'jar:' URL: "
                                        + ex.getMessage());
                            }
                        }
                    }
    ...
    /**
     * Determine the root directory for the given location. <p>Used for
     * determining the starting point for file matching, resolving the root
     ...
     * pattern. <p>Will return "/WEB-INF/" for the pattern "/WEB-INF/*.xml", for
     * example.
     * 
     * @param location
    ...
    /**
     * Resolve the specified resource for path matching. <p>The default
     * implementation detects an Equinox OSGi "bundleresource:" / "bundleentry:"
     * URL and resolves it into a standard jar file URL that can be traversed
     * using Spring's standard jar file traversal algorithm.
     * 
     * @param original
     ...
     * the {@code doFindPathMatchingJarResources} method can handle. <p>The
     * default implementation checks against the URL protocols "jar", "zip" and
     * "wsjar" (the latter are used by BEA WebLogic Server and IBM WebSphere,
