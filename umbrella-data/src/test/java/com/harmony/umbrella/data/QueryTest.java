...
public class QueryTest {
    ...
    public void testGrouping() {
        List<String> v = builder
                .enable(QueryFeature.FULL_TABLE_QUERY)
                .groupBy("content")
                .execute()
                .getResultList("content", String.class);
        assertEquals(1, v.size());
    }

    @Test
    public void testFunctionQuery() {
        assertEquals(1l, builder
                .equal("name", "wuxii")
                .execute()
                .getFunctionResult("count", "name", Long.class)
                .longValue());
    }
    ...
    public void testVoQuery() {
        ModelVo v = builder
                .equal("name", "wuxii")
                .execute()
                .getSingleResult(new String[]{"name", "code", "content"}, ModelVo.class);
        assertEquals("wuxii", v.getName());
    }

    @Test
    public void testVoAndFunctionQuery() {
        List<ModelVo> v = builder
                .enable(QueryFeature.FULL_TABLE_QUERY)
                .groupBy("content")
                .execute()
                .getResultList(new String[]{"max(id)", "content"}, ModelVo.class);
        assertEquals(1, v.size());
    ...
    public void testJoin() {
        List<Model> list = builder.join("subModels", JoinType.INNER)
                .equal("subModels.name", "foo")
                .getResultList();
    ...
    public void testFeature() {
        System.out.println(QueryFeature.isEnabled(0, QueryFeature.CONJUNCTION));
    }
