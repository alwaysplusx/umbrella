...
import static com.harmony.modules.data.domain.Specifications.CompositionType.*;
...
public class Specifications<T> implements Specification<T>, Serializable {
	...
	private static class NegatedSpecification<T> implements Specification<T>, Serializable {
		...
		/**
		 * Creates a new {@link NegatedSpecification} from the given
		 * {@link Specification}
		 * 
		 * @param spec
		 *            may be {@iteral null}
	...
	/**
	 * A {@link Specification} that combines two given {@code Specification}s
	 * via a given {@link CompositionType}.
	 * 
	 * @author Thomas Darimont
	 * @since 1.6
	 */
	private static class ComposedSpecification<T> implements Specification<T>, Serializable {
		...
		/**
		 * Creates a new {@link ComposedSpecification} from the given
		 * {@link Specification} for the left-hand-side and the right-hand-side
		 * with the given {@link CompositionType}.
		 * 
		 * @param lhs
		 *            may be {@literal null}
		 * @param rhs
		...
		/**
		 * Returns {@link Predicate} for the given {@link Root} and
		 * {@link CriteriaQuery} that is constructed via the given
