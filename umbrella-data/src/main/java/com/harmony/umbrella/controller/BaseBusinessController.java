 ...
 */
public abstract class BaseBusinessController<T extends Serializable, ID extends Serializable> implements BusinessController<T, ID> {

    protected final EntityMetadata<T, ID> entityMeta;

    protected BondParser parser = SpecificationTransform.getInstance();

    public BaseBusinessController(Class<T> entityClass) {
    ...
    public void deleteById(ID id) {
        getDao().deleteById(entityMeta.getJavaType(), id);
    }

    @Override
    public void deleteByIds(Iterable<ID> ids) {
        getDao().deleteByIds(entityMeta.getJavaType(), ids);
    }
