...
public class UmbrellaXMLInputFactory extends WstxInputFactory {
    /**
     * Let's limit max size to 3/4 of 16k, since this corresponds to 64k main
     * hash index. This should not be too low, but could perhaps be further
     * lowered?
     */
    final static int MAX_SYMBOL_TABLE_SIZE = 12000;

    /**
     * Number of generations should not matter as much as raw size... but let's
     * still cap it at some number. 500 generations seems reasonable for
     * flushing (note: does not count uses where no new symbols were added).
     */
    ...
    /**
     * 'Root' symbol table, used for creating actual symbol table instances, but
     * never as is.
     */
    ...
    /**
     * Actual current 'parent' symbol table; concrete instances will be created
     * from this instance using <code>makeChild</code> method
     */
    ...
    public UmbrellaXMLInputFactory() {
        mConfig = ReaderConfig.createFullDefaults();
    }

    /**
     * Method that can be used to ensure that specified symbol is contained in
     * the shared symbol table. This may occasionally be useful in
     * pre-populating symbols; although it is unlikely to be commonly useful.
     * 
     * @since 4.2.1
     */
    ...
    /**
     * Method readers created by this factory call, if DTD caching is enabled,
     * to see if an external DTD (subset) has been parsed and cached earlier.
     */
    public synchronized DTDSubset findCachedDTD(DTDId id) {
        return (mDTDCache == null) ? null : (DTDSubset) mDTDCache.find(id);
    }

    // // // Callbacks for updating shared information

    /**
     * Method individual parsers call to pass back symbol table that they
     * updated, which may be useful for other parser to reuse, instead of
     * previous base symbol table.
     * <p>
     * Note: parser is only to call this method, if passed-in symbol table was
     * modified, ie new entry/ies were added in addition to whatever was in root
     * table.
     */
    public synchronized void updateSymbolTable(SymbolTable t) {
        ...
        if (t.isDirectChildOf(curr)) {
             ...
             */
            if (t.size() > MAX_SYMBOL_TABLE_SIZE || t.version() > MAX_SYMBOL_TABLE_GENERATIONS) {
...
//else System.err.println("Debug: skipping symbol table update");
    ...
    public XMLEventReader createXMLEventReader(InputStream in) throws XMLStreamException {
        ...
        return new WstxEventReader(createEventAllocator(), createSR(null, in, null, true, false));
    }

    public XMLEventReader createXMLEventReader(InputStream in, String enc) throws XMLStreamException {
        // false for auto-close, since caller has access to the input stream
        return new WstxEventReader(createEventAllocator(), createSR(null, in, enc, true, false));
    }

    public XMLEventReader createXMLEventReader(Reader r) throws XMLStreamException {
        // false for auto-close, since caller has access to the input stream
        return new WstxEventReader(createEventAllocator(), createSR(null, r, true, false));
    }
    ...
    public XMLEventReader createXMLEventReader(String systemId, InputStream in) throws XMLStreamException {
        // false for auto-close, since caller has access to the input stream
        return new WstxEventReader(createEventAllocator(), createSR(SystemId.construct(systemId), in, null, true, false));
    }
    ...
    public XMLEventReader createXMLEventReader(String systemId, Reader r) throws XMLStreamException {
        ...
        return new WstxEventReader(createEventAllocator(), createSR(SystemId.construct(systemId), r, true, false));
    }
    ...
    public XMLStreamReader createXMLStreamReader(javax.xml.transform.Source src) throws XMLStreamException {
        // false -> not for event. No definition for auto-close; called method
        // will decide
     ...
     * {@link ReaderConfig#setEntityResolver} instead, if possible, since this
     * just wraps passed in resolver.
     */
    ...
    public XMLEventReader2 createXMLEventReader(URL src) throws XMLStreamException {
         ...
         */
        return new WstxEventReader(createEventAllocator(), createSR(createPrivateConfig(), src, true, true));
    }
    ...
    public XMLEventReader2 createXMLEventReader(File f) throws XMLStreamException {
         ...
         */
        return new WstxEventReader(createEventAllocator(), createSR(f, true, true));
    }
    ...
    /**
     * Convenience factory method that allows for parsing a document stored in
     * the specified file.
     */
    ...
    /**
     * Bottleneck method used for creating ALL full stream reader instances (via
     * other createSR() methods and directly)
     *
     * @param forER
     *            True, if the reader is being constructed to be used by an
     *            event reader; false if it is not (or the purpose is not known)
     * @param autoCloseInput
     *            Whether the underlying input source should be actually closed
     *            when encountering EOF, or when <code>close()</code> is called.
     *            Will be true for input sources that are automatically managed
     *            by stream reader (input streams created for
     *            {@link java.net.URL} and {@link java.io.File} arguments, or
     *            when configuration settings indicate auto-closing is to be
     *            enabled (the default value is false as per Stax 1.0 specs).
     */
    private XMLStreamReader2 doCreateSR(ReaderConfig cfg, SystemId systemId, InputBootstrapper bs, boolean forER, boolean autoCloseInput)
     ...
     * <p>
     * Note: defined as public method because it needs to be called by SAX
     * implementation.
     *
     * @param systemId
     *            System id used for this reader (if any)
     * @param bs
     *            Bootstrapper to use for creating actual underlying physical
     *            reader
     * @param forER
     *            Flag to indicate whether it will be used via Event API (will
     *            affect some configuration settings), true if it will be, false
     *            if not (or not known)
     * @param autoCloseInput
     *            Whether the underlying input source should be actually closed
     *            when encountering EOF, or when <code>close()</code> is called.
    ...
    public XMLStreamReader2 createSR(ReaderConfig cfg, SystemId systemId, InputBootstrapper bs, boolean forER, boolean autoCloseInput)
            throws XMLStreamException {
    ...
    protected XMLStreamReader2 createSR(SystemId systemId, InputStream in, String enc, boolean forER, boolean autoCloseInput) throws XMLStreamException {
        ...
        if (enc == null || enc.length() == 0) {
            return createSR(cfg, systemId, StreamBootstrapper.getInstance(null, systemId, in), forER, autoCloseInput);
        }
        ...
        return createSR(cfg, systemId, ReaderBootstrapper.getInstance(null, systemId, r, enc), forER, autoCloseInput);
    }
    ...
    protected XMLStreamReader2 createSR(ReaderConfig cfg, URL src, boolean forER, boolean autoCloseInput) throws XMLStreamException {
        ...
        try {
            return createSR(cfg, systemId, URLUtil.inputStreamFromURL(src), forER, autoCloseInput);
    ...
    private XMLStreamReader2 createSR(ReaderConfig cfg, SystemId systemId, InputStream in, boolean forER, boolean autoCloseInput) throws XMLStreamException {
        return doCreateSR(cfg, systemId, StreamBootstrapper.getInstance(null, systemId, in), forER, autoCloseInput);
    }
    ...
    protected XMLStreamReader2 createSR(SystemId systemId, Reader r, boolean forER, boolean autoCloseInput) throws XMLStreamException {
        return createSR(createPrivateConfig(), systemId, ReaderBootstrapper.getInstance(null, systemId, r, null), forER, autoCloseInput);
    }
        ...
        try {
            ...
            if (!f.isAbsolute()) {
                ...
                if (base != null) {
                    ...
                    return createSR(cfg, SystemId.construct(src), URLUtil.inputStreamFromURL(src), forER, autoCloseInput);
                }
            }
    ...
    /**
     * Another internal factory method, used when dealing with a generic Source
     * base type. One thing worth noting is that 'auto-closing' will be enabled
     * if the input source or Reader is constructed (and thus owned) by
     * Woodstox.
     *
     * @param forER
     *            True, if the reader is being constructed to be used by an
     *            event reader; false if it is not (or the purpose is not known)
     */
    protected XMLStreamReader2 createSR(javax.xml.transform.Source src, boolean forER) throws XMLStreamException {
        ReaderConfig cfg = createPrivateConfig();
        Reader r = null;
        InputStream in = null;
        String pubId = null;
        ...
        if (bs == null) { // may have already created boostrapper...
            if (r != null) {
                ...
                autoCloseInput = true;
                try {
                    return createSR(cfg, URLUtil.urlFromSystemId(sysId), forER, autoCloseInput);
                } catch (IOException ioe) {
                    throw new WstxIOException(ioe);
                }
            } else {
    ...
    protected XMLEventAllocator createEventAllocator() {
         ...
         */
        return mConfig.willPreserveLocation() ? DefaultEventAllocator.getDefaultInstance() : DefaultEventAllocator.getFastInstance();
    }

    /**
     * Method called to construct a copy of the factory's configuration object,
     * such that two will be unlinked (changes to one are not reflect in the
     * other).
     * <p>
     * Note: only public so that other woodstox components outside of this
